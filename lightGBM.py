...
def train_lightgbm_model(train_df):
    ...
    model = lgb.train(
        ...
        # You can add early_stoppmodeling here too
...
def recommend_words(input_word, df, similarity_df, model, word_embeddings, top_n=5):
    ...
    # Get category of the input word (handle missing category)
    category_row = df[df['word'] == input_word]
    if category_row.empty or 'auto_category' not in df.columns:
        print(f"Category for '{input_word}' not found.")
        return []

    category = category_row['auto_category'].iloc[0]

    # Get embedding of the input word
    if input_word not in word_embeddings:
        print(f"Embedding for '{input_word}' not found.")
        return []
    ...
    for word in df['word'].unique():
        ...
        # Get category of candidate word (Handle missing category)
        candidate_row = df[df['word'] == word]
        candidate_category = candidate_row['auto_category'].iloc[0] if not candidate_row.empty else "Unknown"

        # Calculate similarity score from pre-computed similarity matrix (Handle missing similarity)
        precomputed_similarity = similarity_df.loc[input_word, word] if (input_word in similarity_df.index and word in similarity_df.columns) else 0.0

        # Ensure input is a 2D array for model prediction
        model_input = np.hstack((input_embedding.reshape(1, -1), candidate_embedding.reshape(1, -1)))

        # Predict similarity using model
        try:
            model_similarity = model.predict(model_input)[0]
        except Exception as e:
            print(f"Model prediction failed for '{word}': {e}")
            model_similarity = 0.0  # Default to 0 similarity if error occurs

        # Compute final similarity score
        final_similarity = 0.5 * precomputed_similarity + 0.5 * model_similarity

        # Append result
        recommendations.append({
            'word': word,
            'category': candidate_category,
            'same_category': candidate_category == category,
            'similarity': final_similarity,
            'video_link': candidate_row['video_link'].iloc[0] if 'video_link' in df.columns and not candidate_row.empty else "N/A"
        })
    ...
    # If not enough category-based recommendations, fill with others
...
if __name__ == "__main__":
    file_path = "/content/Final_Expanded_1000_Words_dataset.csv"  # Your actual file path
